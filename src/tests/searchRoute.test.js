...
const { batchFilterData } = require('../service/serviceData.js');
const expected1 = [
                    {
...
const expected2 =  [{"name":"The Lord of the Rings: The Return of the King","image":"http://lorempixel.com/640/480","description":"Nihil hic neque dignissimos totam omnis ut aut. Fugiat voluptatem rem quisquam provident est odit. Necessitatibus veniam architecto quia. Rerum deserunt reiciendis velit voluptatem tempora iusto similique. Atque mollitia pariatur quia voluptatem qui laborum laborum rerum molestias.","dateLastEdited":"2018-08-06T08:27:26.187Z"},{"name":"District Solutions Orchestrator","image":"http://lorempixel.com/640/480","description":"Laboriosam occaecati modi sit voluptatem. Quis harum rerum similique at. The Lion King. Et porro eum quia eligendi doloribus aut. Tenetur provident maxime quod illum vitae excepturi. Nemo ipsum non.","dateLastEdited":"2018-10-03T18:56:53.492Z"},{"name":"Human Web Agent","image":"http://lorempixel.com/640/480","description":"Vitae dolor natus aut aut. Totam dolor porro. Rem est repellendus voluptas eos soluta. The Lord of the Rings: The Return of the King","dateLastEdited":"2018-04-16T09:57:36.659Z"},{"name":"The Lion King","image":"http://lorempixel.com/640/480","description":"Fugiat praesentium aspernatur accusantium praesentium blanditiis modi. Ipsam dignissimos odio eum aut fugit aliquam error facilis explicabo. Voluptatum eaque ullam voluptate hic dolorem dolores ab quod. Incidunt consequatur nam et voluptatem reprehenderit quibusdam hic aut. Architecto voluptas numquam est natus quis. Reprehenderit eaque voluptas voluptas nihil cupiditate.","dateLastEdited":"2017-12-28T04:21:00.923Z"}]
const expected4 =   [
                         {
                           name: 'The Lion King',
                           image: 'http://lorempixel.com/640/480',
...
describe("Kodo API",()=>{
    test('should return 200 response when user hits search route without any query parameters', async () => {
        const movieData = await batchFilterData();
        expect(movieData.matchedPhrases).toStrictEqual(expected1);
    }, 1000 * 60 * 2);
    test('should return 200 response when user hits search route with', async () => {
            const movieData = await batchFilterData('the');
            expect(movieData.matchedPhrases).toStrictEqual(expected2);
        }, 1000 * 60 * 2);
    test('should return 200 response when user hits search route with', async () => {
                const movieData = await batchFilterData('"The"');
                expect(movieData.matchedPhrases).toStrictEqual(expected2);
    ...
    test('should return 200 response when user hits search route with', async () => {
                    const movieData = await batchFilterData('"The"', 'asc', 'dateLastEdited');
                    expect(movieData.matchedPhrases).toStrictEqual(expected4);
                }, 1000 * 60 * 2);
    test('should return 200 response when user hits search route with', async () => {
                        const movieData = await batchFilterData('"The"', 'desc', 'name');
                        expect(movieData.matchedPhrases).toStrictEqual(expected5);
                    }, 1000 * 60 * 2);
}, 1000 * 60 * 2)